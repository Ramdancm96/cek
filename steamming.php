<?php
function cari($kata){
    $stemming  = file_get_contents("./stemming.txt");
 	$stemming  = preg_split("/[\s]+/", $stemming);
 	if(in_array($kata, $stemming)){
 		return 1;
 	}else{
 		return 0;
 	}
...
if(cari($kata)!=1){
	...
	if(substr($kata,0,3)=="ber"){
        ...
        if(substr($kata,3,1)=="h"){
            $kata =$kata;
        }
        else if(substr($kata,3,1)=="a" || substr($kata,3,1)=="i" || substr($kata,3,1)=="e" || substr($kata,3,1)=="u" || substr($kata,3,1)=="o"){
			$kata = "r".substr($kata,3);
		}else{
			$kata = substr($kata,3);
		}
